...
!`gh issue list --state open --label ready-for-agent --json number,title,body,comments,labels --limit 100`
...
The fork's Pi + Docker `parallel-planner-with-review` configuration reproduces the Pidex defaults: OpenAI Codex OAuth from `~/.pi/agent/auth.json`, high/low role-specific reasoning, 50 planning cycles, and isolated container-side `node_modules` directories. GitHub Issues templates filter on the canonical `ready-for-agent` label.
...
| `--create-label`          | No       | Interactive prompt           | `true` / `false` — whether to create the `ready-for-agent` GitHub label (only with `--issue-tracker github-issues`) |
...
└── .gitignore      # Ignores .env, logs/, worktrees/, and isolated container dependencies
```
